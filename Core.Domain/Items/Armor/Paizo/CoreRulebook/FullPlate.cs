...
namespace Core.Domain.Items.Armor.Paizo.CoreRulebook
{
    ...
    public sealed class FullPlate : Armor, IFullPlate
    {
        ...
        public FullPlate(SizeCategory size, FullPlateMaterial material)
            : base(9, GetHardnessForMaterial(material))
        {
            const byte ARMOR_CHECK_PENALTY = 6;
            const byte MAX_DEX_BONUS = 1;
            const double WEIGHT = 50;
            const double PRICE = 1500;
            NameFragment standardName = new NameFragment("Full Plate", "http://www.d20pfsrd.com/equipment/Armor/full-plate");
